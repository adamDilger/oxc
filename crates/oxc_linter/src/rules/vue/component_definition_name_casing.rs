use oxc_ast::{AstKind, ast::{Expression, StringLiteral}};
...
impl Rule for ComponentDefinitionNameCasing {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
      // if let AstKind::ExportDefaultDeclaration(object_property) = node.kind() {
      //   println!("Found export default declaration at {:?}", object_property.span);
      // }

      let AstKind::ObjectProperty(object_property) = node.kind() else {
        return;
      };
      ...
      // if the first character is not a capital letter between A-Z, report diagnostic
      ...
      if is_kebab_case {
        ctx.diagnostic_with_fix(
          component_definition_name_casing_diagnostic(identifier_name.span),
           |fixer| {
            fixer.replace(identifier_name.span.shrink(1), kebab_to_pascal(&identifier_name.value))
          }
        );
      }
    }
